...
class PhotoUpload {
    ...
    private function checkIterance($file, $photoObj) {
        ...
        if ($fileMD5 != null) {
            //根据md5值取得图像数据
            $where = "id = '" . $fileMD5 . "'";
            $photoRow = $photoObj->find()->where($where)->one();
        }
        //设置了缩略图
            ...
            if (is_file($photoRow['img'])) {
                ...
                if ($this->thumbWidth && $this->thumbHeight && $this->thumbKey) {
                    foreach ($this->thumbWidth as $thumbWidth_Key => $thumbWidth_Val) {
                        //获取此宽度和高度应有的缩略图名
                        $fileExt = File::getFileSuffix($photoRow['img']);
                    ...
                    foreach ($this->thumbKey as $thumbKey_key => $thumbKey_val) {
                        ...
                        $thumbName = $this->thumb($photoRow['img'], $this->thumbWidth[$thumbKey_key], $this->thumbHeight[$thumbKey_key], $this->fileDir, $thumbExtName);
                        $result['thumb'][$this->thumbKey[$thumbKey_key]] = $thumbName;
                    }
                }
                return $result;
            } else {
                $photoObj->deleteAll('id = "' . $photoRow['id'] . '"');
                return null;
            }
        } else {
            return null;
    ...
    /**
     * 生成$fileName文件的缩略图,位置与$fileName相同
     * @param string  $fileName 要生成缩略图的目标文件
     * @param int     $width    缩略图宽度
     * @param int     $height   缩略图高度
     * @param string  $fileDir  图片文件路径
     * @param string  $extName  缩略图文件名附加值
     * @param string  $saveDir  缩略图存储目录
     */
    public function thumb($fileName, $width, $height, $fileDir, $extName = '_thumb', $saveDir = '') {
        $thumb = new Thumb($fileDir);
    ...
    private function insert($insertData, $photoObj) {
        $photoInfo = $photoObj->find()->where('id = "' . $insertData['id'] . '"')->one();
        if ($this->iterance == true && empty($photoInfo)) {
            $photoObj->id = $insertData['id'];
            $photoObj->img = $insertData['img'];
            $photoObj->save();
        }
    }
        ...
        foreach ($upState as $key => $rs) {
            ...
            foreach ($rs as $innerKey => $val) {
                if ($val['flag'] == 1) {
                    ...
                    $fileMD5 = md5_file($this->fileDir . $fileName);
