...
class Upload {
     ...
     */
    function __construct($fileDir, $size = 10000, $type = array()) {
            ...
            if (is_array($_FILES[$field]['name'])) {
                ...
                foreach ($keys as $key) {
                    ...
                    if (isset($_FILES[$field]['error'][$key]) && $_FILES[$field]['error'][$key] != 0) {
                        ...
                        //图片木马检测
                        if (in_array($fileext, $this->checkType) && !Filter::checkHex($_FILES[$field]['tmp_name'][$key])) {
                            $fileInfo[$key]['flag'] = -9;
                        } else {
                            /* 开始上传文件 */
                            //(1)上传类型不符合
                            ...
                            //(3)成功情况
                                ...
                                if ($this->isForge == false) {
                                    if (is_uploaded_file($_FILES[$field]['tmp_name'][$key])) {
                                        File::mkdir($this->fileDir . $this->dir);
                                        move_uploaded_file($_FILES[$field]['tmp_name'][$key], $this->fileDir . $this->dir . $fileInfo[$key]['name']);
                                    }
                                } else {
                                    File::xcopy($_FILES[$field]['tmp_name'][$key], $this->fileDir . $this->dir . $fileInfo[$key]['name']);
                                }
                            }
                        }
                    }
                ...
                if (isset($_FILES[$field]['error']) && $_FILES[$field]['error'] != 0) {
                    ...
                    //图片木马检测
                    if (in_array($fileext, $this->checkType) && !Filter::checkHex($_FILES[$field]['tmp_name'])) {
                        $fileInfo[0]['flag'] = -9;
                    } else {
                        /* 开始上传文件 */
                        //(1)上传类型不符合
                        ...
                        //(3)成功情况
                            ...
                            if ($this->isForge == false) {
                                if (is_uploaded_file($_FILES[$field]['tmp_name'])) {
                                    File::mkdir($this->fileDir . $this->dir);
                                    move_uploaded_file($_FILES[$field]['tmp_name'], $this->fileDir . $this->dir . $fileInfo[0]['name']);
                                }
                            } else {
                                File::xcopy($_FILES[$field]['tmp_name'], $this->fileDir . $this->dir . $fileInfo[0]['name']);
                            }
                        }
                    }
                }
